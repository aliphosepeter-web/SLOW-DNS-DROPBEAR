...
SSHD_PORT=2222
SLOWDNS_PORT=5300
GITHUB_BASE="https://raw.githubusercontent.com/aliphosepeter-web/SLOW-DNS-DROPBEAR/main/DNSTT%20MODED"
...
# ============================================================================
print_step "1"
print_info "Installing and configuring Dropbear on port $SSHD_PORT"

echo -ne "  ${CYAN}Installing Dropbear...${NC}"
apt update > /dev/null 2>&1 && apt install -y dropbear > /dev/null 2>&1 &
show_progress $!
echo -e "\r  ${GREEN}Dropbear installed${NC}"
...
show_progress $!
echo -e "\r  ${GREEN}Dropbear running on port $SSHD_PORT${NC}"
